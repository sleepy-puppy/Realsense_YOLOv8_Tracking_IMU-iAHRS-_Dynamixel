...
class RealSenseObjectTracker(QMainWindow):
    ...
    def initialize_realsense(self):
        ...
        print(f"연결된 장치 수: {len(connected_devices)}")

        # 연결된 장치들에 대한 정보 출력
        device_list = []
        for i, device in enumerate(connected_devices):
            if device.get_info(rs.camera_info.name) == 'Platform Camera':
                continue  # 내장 카메라는 제외
            print(f"디바이스 {i+1}:")
        ...
        # 디바이스 번호 입력
        selected_device_index = int(input(f"사용할 카메라 번호를 선택하세요 (1 ~ {len(device_list)}): ")) - 1

        # 선택한 디바이스의 시리얼 번호로 카메라 설정
        selected_device = device_list[selected_device_index]
    ...
    def update_buttons_and_labels(self, person_tracks, dangerous_person_id):
        ...
        for track in person_tracks:
            ...
            if person_id == dangerous_person_id:
                ...
                if person_id in self.buttons:
                    self.buttons[person_id].setStyleSheet("background-color: red")
            else:
                ...
                # 위험 인물이 아닌 경우 버튼 색을 기본값으로 변경
                if person_id in self.buttons:
                    self.buttons[person_id].setStyleSheet("")
