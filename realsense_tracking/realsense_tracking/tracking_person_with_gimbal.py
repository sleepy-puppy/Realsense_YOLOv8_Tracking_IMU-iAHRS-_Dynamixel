...
class DepthSubscriber(Node):
    ...
    def target_person_depth_listener_callback(self, msg):
        ...
        if self.current_yaw is not None and depth is not None:
            # yaw 값을 받아서 오메가 계산
            ...
            print(f"ID {person_id}: yaw 또는 depth 값이 유효하지 않음 (current_yaw={self.current_yaw}, depth={depth})")

        
    def yaw_callback(self, msg):
        self.current_yaw = msg.data if msg.data is not None else None
